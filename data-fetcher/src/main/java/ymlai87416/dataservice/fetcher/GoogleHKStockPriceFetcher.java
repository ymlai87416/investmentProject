 ...
 */
@Component("GoogleHKStockPriceFetcher")
public class GoogleHKStockPriceFetcher /*implements Fetcher*/{
    ...
    public boolean run(Map<String, Object> parameter) {
        try{
            log.info("Downloading stock quote from Google Finance");
    ...
    //TODO: update to use goolge library
    private YahooHKStockPriceFetcher.DateRange decideDailyPriceDownloadDateRange(Symbol symbol){
        java.sql.Date latestDate = dailyPriceService.getLastestDailyPriceDateForSymbol(symbol);
        if(latestDate == null)
            return new YahooHKStockPriceFetcher.DateRange(startDate, endDate);
        else if(latestDate.compareTo(endDate) == 0)
            return null;
        else{
            return new YahooHKStockPriceFetcher.DateRange(latestDate, endDate);
        }
    }

    //TODO: update to use google library
    private List<Symbol> firstRoundDownloadDailyPrice(List<Symbol> allSymbols){
        ...
        for(Symbol symbol : allSymbols){
            ...
            try{
                ...
                YahooHKStockPriceFetcher.DateRange dateRange = decideDailyPriceDownloadDateRange(symbol);
    ...
    private List<Symbol> retryDownloadDailyPrice(List<Symbol> redownloadDueToAdjustment){
        ...
        for(Symbol symbol: redownloadDueToAdjustment){
            ...
            try {
                ...
                //download symbol
                List<DailyPrice> dailyPrices = getDailyPriceFromGoogle(symbol, dateRange);
    ...
    //TODO: update google library
    private List<DailyPrice> getDailyPriceFromGoogle(Symbol symbol, YahooHKStockPriceFetcher.DateRange dateRange) throws IOException {
